...
touch current_targets_spd_RLS_1
touch current_target_RLS_1
touch current_target_temp_RLS_1

sleep 1.2
while true; do
	# обработка новой пачки координат целей
	cp current_target_RLS_1 current_target_temp_RLS_1
	...
	for file in $files; do
		...
		if target_in_sector $X $Y; then
			if grep -q "$target" current_target_RLS_1; then         # проверка была ли обнаружена эта цель ранее
				if ! grep -q "$target" current_targets_spd_RLS_1; then # проверка была ли скорость уже рассчитана

					# ---------------------------рассчет скорости --------------------------------------
					X_old=$(cat current_target_temp_RLS_1 | grep -e "$target" | cut -d ',' -f 2)
					Y_old=$(cat current_target_temp_RLS_1 | grep -e "$target" | cut -d ',' -f 3)
					X_delt=$((X - X_old))
					Y_delt=$((Y - Y_old))
					...
					if ((($Y_delt != 0) && $X_delt != 0)); then # проверка на случай поступления старых координат
						...
						echo "$target" >>current_targets_spd_RLS_1
	...
	for dead_target in $(cat current_target_temp_RLS_1); do
		dead_target_id=$(echo $dead_target | cut -d ',' -f 1)
		echo -e "\nЦель ID: $dead_target_id пропала с радара"
		sed -i "/$dead_target/d" current_target_RLS_1
	done
	#------------------------------------------------------------------------------------------
	sleep 0.4
done
