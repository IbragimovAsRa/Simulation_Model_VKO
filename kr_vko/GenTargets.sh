...
while :; do
	let NoTarget-=1
	if [ $NoTarget -lt 0 ]; then
		NoTarget=$MaxKolTargets
		sleep $Sleeptime
		find -mmin +1 -delete 2>/dev/null
		#map
		if [ "$1" == "map" ] || [ "$1" == "-map" ]; then
			for i in $(seq 0 $MaxKolTargets); do
				if [ ${TargetsId[3 + 10 * $i]} -ge 0 ]; then
			...
			clear
			for y in $(seq 44 -1 0); do
				for x in $(seq 0 130); do
	...
	if [[ "${#TargetsId[0 + 10 * $NoTarget]}" -lt 1 ]]; then #Генерация цели
		...
		Xplus=$(($Xmax - $Xmin))
		Yplus=$(($Ymax - $Ymin))
		Xkoord=$(($RANDOM % ($Xplus / 1000) * 1000 + $Xmin))
		Ykoord=$(($RANDOM % ($Yplus / 1000) * 1000 + $Ymin))
		#Xkoord=$((12900000   ))
		#Ykoord=$((1   ))
		BASE_STR=$(mcookie)
		NameTarget=${BASE_STR:11:6}
		...
		if [ -e "$DestroyDirectory/${TargetsId[0 + 10 * $NoTarget]}" ]; then #Уничтожение цели по запросу
			rm "$DestroyDirectory/${TargetsId[0 + 10 * $NoTarget]}"
			if [ $((RANDOM % $Probability)) -ge 1 ]; then
		...
		if [ ${TargetsId[5 + 10 * $NoTarget]} -le 0 ]; then #Время жизни цели истекло
