...
rm -rf current_target_zrdn_2 current_targets_spd_zrdn_2
rm -rf current_target_temp_zrdn_2

touch current_targets_spd_zrdn_2
touch current_target_zrdn_2
touch current_target_temp_zrdn_2

sleep 1.2

while true; do
	# обработка новой пачки координат целей
	cp current_target_zrdn_2 current_target_temp_zrdn_2
	...
	for file in $files; do
		...
		if target_in_sector $X $Y; then
			if grep -q "$target" current_target_zrdn_2; then
				...
				if ! grep -q "$target" current_targets_spd_zrdn_2; then

					# ---------------------------рассчет скорости --------------------------------------
					X_old=$(cat current_target_temp_zrdn_2 | grep -e "$target" | cut -d ',' -f 2)
					Y_old=$(cat current_target_temp_zrdn_2 | grep -e "$target" | cut -d ',' -f 3)
					X_delt=$((X - X_old))
					Y_delt=$((Y - Y_old))
					if ((($Y_delt != 0) && $X_delt != 0)); then # проверка на случай поступления старых координат
						...
						echo "$target" >>current_targets_spd_zrdn_2
					fi
				fi
				sed -i "/$target/d" current_target_temp_zrdn_2
			else
				echo -e "\n\033[31mОбнаружена цель ID:$target с координатами x=$X, y=$Y\033[0m"
				echo "$target,$X,$Y" >>current_target_zrdn_2
			fi
		fi
	done

	#----------------------обработка мертвых целей---------------------------------------------
	for dead_target in $(cat current_target_temp_zrdn_2); do
		dead_target_id=$(echo $dead_target | cut -d ',' -f 1)
		echo -e "\nЦель ID: $dead_target_id пропала с радара"
		sed -i "/$dead_target/d" current_target_zrdn_2
	done
	#------------------------------------------------------------------------------------------
	sleep 0.4
done
